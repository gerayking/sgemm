...
__global__ void Sgemm(
    ...
    float frag_a_tmp[THREAD_SIZE_Y];
    ...
    do
    {
        ...
        for(int i=0;i<BLOCK_SIZE_M;i+=A_TILE_ROW_STRIDE){
            ...
            FETCH_FLOAT4(frag_a_tmp[ldg_a_index]) = FETCH_FLOAT4(A[OFFSET(
                A_TILE_ROW_START + i,
                A_TILE_COL + tile_idx,
                K
            )]);
            As[A_TILE_COL][A_TILE_ROW_START + i] = frag_a_tmp[ldg_a_index];
            As[A_TILE_COL + 1][A_TILE_ROW_START + i] = frag_a_tmp[ldg_a_index + 1];
            As[A_TILE_COL + 2][A_TILE_ROW_START + i] = frag_a_tmp[ldg_a_index + 2];
            As[A_TILE_COL + 3][A_TILE_ROW_START + i] = frag_a_tmp[ldg_a_index + 3];
        }
        for(int i=0;i<BLOCK_SIZE_K;i+=B_TILE_ROW_STRIDE){
            FETCH_FLOAT4(Bs[B_TILE_ROW_START + i][B_TILE_COL]) = FETCH_FLOAT4(B[OFFSET(
                i + tile_idx + B_TILE_ROW_START,
                B_TILE_COL,
                N
            )]);
        }
