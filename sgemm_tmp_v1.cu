...
const int M,const int N,const int K){
    ...
    int THREAD_X_PER_BLOCK = blockDim.x;
    int THREAD_Y_PER_BLOCK = blockDim.y;
    int THREAD_NUM_PER_BLOCK = THREAD_X_PER_BLOCK * THREAD_Y_PER_BLOCK;
    ...
    do{
        ...
        for(int i=0;i<BLOCK_SIZE_M;i+=A_TILE_ROW_STRIDE){
            ...
            FETCH_FLOAT4(a_tmp[ldg_a_index]) = FETCH_FLOAT4(A[OFFSET(
                A_TILE_ROW_START + i,
                A_TILE_COL + tile_idx,
                K
            )]);

            As[A_TILE_COL][A_TILE_ROW_START + i] = a_tmp[ldg_a_index];
            As[A_TILE_COL + 1][A_TILE_ROW_START + i] = a_tmp[ldg_a_index+1];
            As[A_TILE_COL + 2][A_TILE_ROW_START + i] = a_tmp[ldg_a_index+2];
            As[A_TILE_COL + 3][A_TILE_ROW_START + i] = a_tmp[ldg_a_index+3];
        }
        for(int i=0; i<BLOCK_SIZE_K; i+=B_TILE_ROW_STRIDE){
            FETCH_FLOAT4(Bs[B_TILE_ROW_START + i][B_TILE_COL]) = FETCH_FLOAT4(B[OFFSET(
                i + B_TILE_ROW_START + tile_idx,
                B_TILE_COL,
                N
            )]);
        }
        ...
        for(int i=0;i<BLOCK_SIZE_K;i++){
            ...
            for(int thread_y = 0; thread_y < THREAD_SIZE_Y; thread_y+=4){
                FETCH_FLOAT4(frag_a[thread_y]) = FETCH_FLOAT4(As[i][THREAD_SIZE_Y * ty + thread_y]);
            }
            for(int thread_x = 0; thread_x < THREAD_SIZE_X; thread_x+=4){
                FETCH_FLOAT4(frag_b[thread_x]) = FETCH_FLOAT4(Bs[i][THREAD_SIZE_X * tx + thread_x]);
            }
