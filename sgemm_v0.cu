...
__global__ void sgemm( 
    ...
    do{
        ...
        if(tile_idx < K){
            // prefetch from global memory to shard memory
            #pragma unroll
            for(int i=0; i < BLOCK_SIZE_M;i+= A_TILE_ROW_STRIDE){
                int ldg_index = i / A_TILE_ROW_STRIDE * 4;
...
int main(int argc,char ** argv){
    ...
    for(int iter=0;iter<nIter;++iter){
        ...
        sgemm<BLOCK_SIZE_M,BLOCK_SIZE_K,BLOCK_SIZE_N,THREAD_SIZE_Y,THREAD_SIZE_X> <<<dimGrid,dimBlock>>>(d_A,d_B,d_C,M,N,K);
    }
